...
Отрицательный счёт — валидный результат: после серии штрафов от шипов игрок реально уходит в минус, и эта запись попадает в лидерборд как есть. Серверный sanity-cap проверяет только верхнюю границу (`> 200`), знак не валидируется.
